...
# Load the Fashion MNIST dataset with normalization
...
train_loader = DataLoader(Subset(fashion_mnist_train, range(num_train)), batch_size=batch_size, shuffle=True)
valid_loader = DataLoader(Subset(fashion_mnist_train, range(num_train, num_train + num_valid)), batch_size=batch_size, shuffle=False)
...
def load_data(loader):
    ...
    for images, labels in loader:
        X.append(images.view(images.size(0), -1))  # Flatten the images to shape (batch_size, 784)
        y.append(labels)
...
# Add these lists before the training loop
train_accuracies = []
valid_accuracies = []
